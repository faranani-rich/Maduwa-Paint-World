...
document.addEventListener("DOMContentLoaded", () => {
  initAuthListener(async ({ user, profile }) => {
    ...
    /* ROLES -------------------------------------------------------- */
    const types = Array.isArray(profile.employeeTypes) ? profile.employeeTypes : [];
    const isOwner = types.includes("owner");
    const isPM = types.includes("pm");
    const isAccountant = types.includes("accountant");

    const canEdit = isOwner || isPM || isAccountant;
    const canDelete = isOwner; // (future use)
    ...
    function setupAddHandler(section, tableId, fields, addBtn, types = {}) {
      ...
      addBtn.addEventListener("click", () => {
        ...
        fields.forEach((f) => {
          const type =
            types[f] || (f.toLowerCase().includes("date") ? "date" : "text");
    ...
    function updateTotals() {
      ...
      `;
      /* re-attach mirrors because innerHTML wipes them */
      totalsEl.querySelectorAll("p").forEach((p) => attachPrintMirror(p));
      renderTable("employees", "employees-table", EMPLOYEE_FIELDS, {
        hours: "number",
        overtimeHours: "number",
        normalRate: "number",
